...
void AudioDeviceManager::openStream(int index) {
    ...
    inputParameters.channelCount              = device->maxInputChannels; // get left and right input
    inputParameters.device                    = index;
    inputParameters.hostApiSpecificStreamInfo = NULL;
    inputParameters.sampleFormat              = paFloat32;
    inputParameters.suggestedLatency          = device->defaultLowInputLatency;

    err = Pa_OpenStream(std::out_ptr(stream), &inputParameters, nullptr, device->defaultSampleRate, 512, paNoFlag, paCallback, NULL);
